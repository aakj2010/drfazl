...
const Search = ({ setActiveTab }) => {
  ...
  const languageContext = useContext(LanguageContext);
  const [selectedLanguage] = useState(languageContext.language);
  const debouncedSearchQuery = useDebounce(searchQuery, 300); // Debounce search query with 300ms delay
  ...
  const handleClick = (number) => {
    ...
    timeoutRef.current = setTimeout(() => {
      navigate(`/chapters#${number}`);
    }, 500); // Add a 500ms delay before navigating
  };
  ...
  const renderVerses = (chapter) => {
    if (chapter) {
      if (selectedDataset) {
        if (chapter.verses && debouncedSearchQuery.trim() !== "") {
          const filteredVerses = chapter.verses.filter(
            (verse) =>
              verse.number.includes(debouncedSearchQuery) ||
              verse.text.toLowerCase().includes(debouncedSearchQuery.toLowerCase())
          );
  ...
  const filteredData = useMemo(() => {
    if (debouncedSearchQuery && chapters && chapters.length > 0) {
      return chapters.filter((chapter) => {
        if (selectedDataset === "quran") {
          const chapterTitleMatches =
            chapter.title && chapter.title.toLowerCase().includes(debouncedSearchQuery.toLowerCase());
          const verses =
            chapter.verses &&
            chapter.verses.filter(
              (verse) =>
    ...
    return [];
  }, [chapters, debouncedSearchQuery, selectedDataset]);
